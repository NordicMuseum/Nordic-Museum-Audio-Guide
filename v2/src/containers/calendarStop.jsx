...
class CalendarStop extends Component {
  ...
  //   calendarStop: PropTypes.object.isRequired,
  ...
  render() {
    ...
    if (!this.props.calendarStop) {
      return null;
    }

    let { calendarStop } = this.props;

    const { playTrack, togglePausePlay } = this.props.actions;

    const duration = translate(calendarStop.duration);
    const floor = translate("floor" + calendarStop.floor + "_Label");

    // let accessibilityLabel;

    // if (calendarStop.shortCreditAccessibilityLabel) {
    //   accessibilityLabel = parseVoiceoverText(calendarStop.shortCreditAccessibilityLabel);
    // } else {
    //   accessibilityLabel = parseVoiceoverText(I18n.t(calendarStop.shortCredit));
    // }
    ...
    let imageURL = this.props.calendarStop.imageURL;
    if (
      this.props.locale === "svKids" &&
      this.props.calendarStop.title === "tours_highlights_title"
    ) {
    ...
    return (
      ...
      >
        <NavigationBarParallax
          ...
          navBarTitle={parseDisplayText(translate(calendarStop.title))}
        ...
        >
            ...
            <View style={styles.playAllButtonContainer}>
              <TouchableOpacity
                ...
                onPress={() => {
                  playTrack(
                    this.props.calendarStop,
                    this.props.calendarStop.audioContent[0].uuid,
                    true
                  );
                }}
              >
          ...
          <AudioContentList
            calendarStop={this.props.calendarStop}
